...
export function filterLogsWithTopics(logs: providers.Log[], topic: any, contractAddress: string) {
  return logs
...
export function compareArgs(actual: any, expected: any): boolean {
  ...
  if ((Array.isArray(actual) && !Array.isArray(expected)) || (!Array.isArray(actual) && Array.isArray(expected))) {
    return false;
  }
