...
export async function toHaveEmitted(transaction: Transaction, contract: Contract, eventName: string) {
  const receipt = await contract.provider.getTransactionReceipt(transaction.hash as string);
  ...
  if (pass) {
    return {
      pass: true,
      message: () => `Expected event "${eventName}" NOT to be emitted, but it was`
    };
  } else {
    return {
      pass: false,
      message: () => `Expected event "${eventName}" to be emitted, but it wasn't`
    };
  }
}
