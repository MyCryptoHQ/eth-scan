...
export async function toChangeBalances(
  ...
  const actualChanges = balancesAfter.map((balance, ind) => balance.sub(balancesBefore[ind]));
  const pass = actualChanges.every((change, ind) => change.eq(BigNumber.from(balanceChanges[ind])));

  const walletsAddresses = wallets.map(wallet => wallet.address);
  if (pass) {
    return {
      pass: true,
      message: () => `Expected ${walletsAddresses} to not change balance by ${balanceChanges} wei,`
    };
  } else {
    return {
      pass: false,
