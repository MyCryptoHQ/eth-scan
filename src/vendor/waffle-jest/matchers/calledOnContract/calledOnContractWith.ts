...
import { validateContract, validateMockProvider, validateFnName } from './calledOnContractValidators';

export function toBeCalledOnContractWith(fnName: string, contract: Contract, parameters: any[]) {
  ...
  const pass = callHistory.some(call => call.address === contract.address && call.data === funCallData);
