...
describe('utils/batch', () => {
  ...
  describe('batch()', () => {
    ...
    it('should batch function calls', async () => {
      ...
      const balances = await batch(spy, 2, ['0x0', '0x1', '0x2', '0x3'], ETHER_BALANCES_ID);

      expect(balances.length).to.equal(4);
      expect(spy).to.have.been.called.exactly(2);
      expect(spy)
        .on.nth(1)
        .to.have.been.called.with(['0x0', '0x1'], ETHER_BALANCES_ID);
      expect(spy)
        .on.nth(2)
        .to.have.been.called.with(['0x2', '0x3'], ETHER_BALANCES_ID);
    });
  });
});
